...
public class GameButton1Script : MonoBehaviour
{
    //image reference
    Image itemImage;
    Image cooldownImage;
    ...
    void Awake()
    {
        ...
        //-----testing hack--------
        //GameManager.Instance.PlayerInventory.AddItem(ItemType.EnergyShield, 10);

        //set info
        if (GameManager.Instance.PlayerInventory.ViewItemCount(ItemType.EnergyShield) > 0)
        {
            cooldownImage.fillAmount = 0f;
            numberText.text = GameManager.Instance.PlayerInventory.ViewItemCount(ItemType.EnergyShield).ToString();
        }
        else
        {
    ...
    private void Update()
    {
        ...
        if (!GameManager.Instance.Paused)
        {
            ...
            {
                //button press number
                if (InputManager.Instance.GetButtonDown(PlayerAction.Button1))
                {
                    //process item click
                    OnButtonClick();
                }
                ...
                if (GameManager.Instance.PlayerInventory.ViewItemCount(ItemType.EnergyShield) > 0)
                {
                    ...
                    if (cooldownTimer > 0)
                    {
                        ...
                        cooldownImage.fillAmount = cooldownTimer / Constants.ENERGY_SHIELD_COOLDOWN;
                        flashButton = true;
                    }
                    else
                    {
        ...
        if (!GameManager.Instance.Paused)
        {
            ...
            if (GameManager.Instance.Player.State == PlayerScript.PlayerState.Manual)
            {
                ...
                if (GameManager.Instance.PlayerInventory.ViewItemCount(ItemType.EnergyShield) > 0)
                {
                    ...
                    {
                        //spawn object
                        Instantiate(Resources.Load<GameObject>("Prefabs/Projectiles and Powerups/EnergyShield"), GameManager.Instance.Player.transform.position, Quaternion.identity);

                        //subtract inventory
                        GameManager.Instance.PlayerInventory.RemoveItem(ItemType.EnergyShield, 1);

                        //update text
                        numberText.text = GameManager.Instance.PlayerInventory.ViewItemCount(ItemType.EnergyShield).ToString();

                        //reset timer and fill amount
                        cooldownTimer = Constants.ENERGY_SHIELD_COOLDOWN;
                        cooldownImage.fillAmount = cooldownTimer / Constants.ENERGY_SHIELD_COOLDOWN;
                    }
                }
                else
